...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        ...
        # resolution of the discretization of state space (cell/m)
        self.resolution = resolution
        self.x_init = self.snap_to_grid(x_init)    # initial state
        self.x_goal = self.snap_to_grid(x_goal)    # goal state

        self.closed_set = set()    # the set containing the states that have been visited
        # the set containing the states that are condidate for future expension
        self.open_set = set()
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        for i in [x[0] - self.resolution, x[0], x[0] + self.resolution]:
            for j in [x[1] - self.resolution, x[1], x[1] + self.resolution]:
                pos = self.snap_to_grid((i, j))
                if self.is_free(pos) and pos != x:
    ...
    def plot_path(self, fig_num=0):
        ...
        plt.plot(solution_path[:, 0], solution_path[:, 1], color="green",
                 linewidth=2, label="A* solution path", zorder=10)
        plt.scatter([self.x_init[0] * self.resolution, self.x_goal[0] * self.resolution], [self.x_init[1]
                                                                                           * self.resolution, self.x_goal[1] * self.resolution], color="green", s=30, zorder=10)
        plt.annotate(r"$x_{init}$", np.array(self.x_init) *
                     self.resolution + np.array([.2, .2]), fontsize=16)
        plt.annotate(r"$x_{goal}$", np.array(self.x_goal) *
                     self.resolution + np.array([.2, .2]), fontsize=16)
        ...
        while len(self.open_set):
            ...
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh in self.closed_set:
                    continue
                tentative_cost_to_arrive = self.cost_to_arrive[x_current] + self.distance(
                    x_current, x_neigh)
                ...
                self.est_cost_through[x_neigh] = tentative_cost_to_arrive + \
                    self.distance(x_neigh, self.x_goal)
            if iteration>=80/self.resolution:
                break
        return False
        ########## Code ends here ##########
